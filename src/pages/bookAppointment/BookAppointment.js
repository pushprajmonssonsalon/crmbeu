...
console.log({membershipitem})
  ...
  const applyMemberShip = () => {
    ...
    const data = {
      ...
      creditsUsed: memberShipStatus ? (subTotalService) : (subtotalPrice-countdiscount),
...
{/* MEMBERSHIP CODE */}
        ...
        {/* {
          visible && phoneNumber?.length>0 && (
            <div style={{}} className="absolute top-[330px] ml-8 h-[104px] w-[283px] overflow-auto bg-white shadow-lg rounded-lg z-10" >
            {userData.length > 0 &&
              userData?.map((item) => {
                return (
                  <div
  ...
  <tbody>
    {productDataReducer?.map((item, index) => (
      ...
      <tr key={index}>
        ...
        <td>{item?.brand}</td>
        <td>{item?.quantity}</td>
        <td> <MdDeleteOutline onClick={() => deleteProduct(index)}  className="text-xl text-red-600 font-bold cursor-pointer"/></td>
      </tr>
    ))}
  </tbody>
</table>
...
<div className="search-container">
          ...
          searchProduct?.length>0 && (
            <div style={{}} className="absolute top-16 ml-8 h-[104px] w-[283px] overflow-auto bg-slate-300 shadow-lg " >
            {
...
<table className="styled-table">
  ...
  <tbody>
    {showSearchProduct?.filter((item) => item.itemId === selectedProduct)?.map((item, index) => (
      <tr key={index}>
        <td><input
                  value={item.name}
                ...
                >
                  ...
                  {staffData?.map((item) => (
                    <option
                      ...
                      value={item._id}
                      style={{ width: "300px" }}
                      onChange={(e)=>setProductStaff(e.target.value)}
                    >
                      {item.name}
                    </option>
                  ))}
        ...
        <td><button
            ...
            onClick={()=>addproductPress(item,productQnt,serviceSelection.staffId)}
          >
            <label
        ...
        <h1 className="text-4xl font-bold mb-10 mt-10 text-center text-green-600">BOOK APPOINTMENT</h1>
          ...
          <div className="flex justify-start gap-3 items-center">
            ...
            <input type="number" className="outline-none" value={discount} onChange={(e) => setDiscount(e.target.value)}/>
            <button onClick={applyDiscount} className="bg-black">Apply Discount</button>
          </div>
          ...
          <select className="mx-3 w-80 h-10 outline-none border-2 border-gray-500 rounded-lg" onChange={membershipPress} disabled={memberShipStatus ? true : false}>
          ...
          {
            memberShipStatus ? (
              <button onClick={applyMemberShip} className="bg-red-600 hover:bg-red-500" >Remove  Membership</button>
            ): (
              <button onClick={applyMemberShip} className="bg-black hover:bg-gray-800">Apply  Membership</button>
            )
          }

          <div className="flex ml-6">
            <h3 className="text-lg font-bold text-black">BALANCE : <span className="text-green-600 font-bold">{membershipCoin}</span></h3>
          </div>
